...
sol_storage! {
    ...
    pub struct LiquidBookEngine {
        ...
        address bitmap_storage_address;
...
sol_interface! {
    ...
    interface IMatcher {
        function execute((uint256,uint256,uint256)[] valid_orders, uint256 incoming_order_quantity) external view returns (uint256);
    }
}
...
impl LiquidBookEngine {
    ...
    pub fn initialize(&mut self, tick_manager_address: Address, order_manager_address: Address, bitmap_storage_address: Address, matcher_address: Address) {
        self.tick_manager_address.set(tick_manager_address);
        self.order_manager_address.set(order_manager_address);
        self.bitmap_storage_address.set(bitmap_storage_address);
        self.matcher_address.set(matcher_address);
    }
    ...
    pub fn place_order(
        ...
        let bitmap_storage_address = self.bitmap_storage_address.get();
        let matcher_address = self.matcher_address.get();
        let tick_manager = ITickManager::new(tick_manager_address);
        let order_manager = IOrderManager::new(order_manager_address);
        let bitmap_storage = IBitmapStorage::new(bitmap_storage_address);
        let matcher = IMatcher::new(matcher_address);

        let mut remaining_incoming_order_volume: alloy_primitives::Uint<256, 4> =
            incoming_order_volume;
        let possible_ticks: Vec<U256> = bitmap_storage.top_n_best_ticks(&*self, incoming_order_is_buy).unwrap();
        ...
        if !filtered_possible_ticks.is_empty() {
            ...
            for tick in filtered_possible_ticks {
                ...
                if !orders.is_empty() {
                    remaining_incoming_order_volume = matcher.execute(&*self, orders, remaining_incoming_order_volume).unwrap();
                }
